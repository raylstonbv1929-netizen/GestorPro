...
import {
    ...
    Building2, FileText, Landmark, Wallet, TrendingUp, SlidersHorizontal
} from 'lucide-react';
    ...
    return (
        <div className="animate-fade-in space-y-6 h-full flex flex-col p-2 overflow-y-auto custom-scrollbar pb-10">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-6 bg-slate-900/40 p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-800/60 shadow-2xl backdrop-blur-xl relative overflow-hidden group">
                ...
                <div className="flex gap-4 relative z-10 lg:w-auto w-full">
                    ...
                    <button
                        ...
                        className={`px-6 py-5 rounded-2xl border transition-all flex items-center gap-3 font-black text-[10px] tracking-widest uppercase italic group ${isFilterPanelOpen ? 'bg-blue-500/10 border-blue-500 text-blue-400' : 'bg-slate-900 border-slate-800 text-slate-500 hover:text-white hover:border-slate-700'}`}
                    >
            ...
            <TacticalFilterBlade
                ...
                title="Scanner de Varredura de Clientes"
                ...
                progress={(filteredClients.length / Math.max(clients.length, 1)) * 100}
                metrics={[
                    { label: 'ENTIDADES LOCALIZADAS', value: filteredClients.length.toString().padStart(3, '0') },
                    { label: 'FIDELIDADE MÉDIA', value: `${stats.percent.toFixed(0)}%` }
                ]}
            >
                <div className="space-y-8">
                    <div className="space-y-4">
                        <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest ml-1">Status Operacional</label>
                        <div className="grid grid-cols-3 gap-3">
                            {['all', 'active', 'inactive'].map(s => (
                                <button
                                    key={s}
                                    onClick={() => setFilter(s)}
                                    className={`px-4 py-4 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border ${filter === s ? 'bg-blue-600 text-white border-blue-400 shadow-lg' : 'bg-slate-900 border-slate-800 text-slate-500 hover:text-slate-200'}`}
                                >
                    ...
                    <div className="p-6 bg-blue-500/5 rounded-2xl border border-blue-500/10 space-y-3">
                        <p className="text-[8px] text-blue-400 font-black uppercase tracking-widest italic flex items-center gap-2">
                            <Info size={12} /> Dica de Busca
                        </p>
                        <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest italic leading-relaxed">
                            Você pode buscar por Nome, CNPJ ou Cidade diretamente no HUD de busca global acima.
                        </p>
                    </div>
                </div>
            </TacticalFilterBlade>
            ...
            <Modal
                ...
                maxWidth="max-w-5xl"
            >
                <Card variant="glass" className="relative z-10 p-0 overflow-hidden border-blue-500/20 shadow-2xl rounded-[1.5rem] md:rounded-[3rem] !scale-100 flex flex-col h-[95vh] md:h-[90vh]">
                                    ...
                                    <div className="bg-slate-900/40 border border-slate-800 p-8 rounded-[2rem] space-y-6">
                                        ...
                                        <div className="space-y-5">
                                            ...
                                            <div className="space-y-1.5">
                                                ...
                                                <select value={form.status} onChange={e => setForm({ ...form, status: e.target.value as 'active' | 'inactive' })} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-xs font-bold text-white outline-none focus:border-blue-500 transition-all appearance-none cursor-pointer">
