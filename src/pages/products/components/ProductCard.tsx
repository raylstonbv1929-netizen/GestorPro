...
import { Warehouse, DollarSign, MapPin, Info, Edit, Trash2, Box, History, ShieldCheck, Zap, ArrowRight, Activity } from 'lucide-react';
                ...
                <div className="flex items-start gap-5 mb-8">
                    ...
                    <div className="min-w-0 flex-1">
                        <div className="flex justify-between items-start">
                            <h4 className="font-black text-white text-lg uppercase tracking-tighter italic leading-none truncate group-hover:text-emerald-300 transition-colors uppercase">{prod.name}</h4>
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                            <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-[0.1em] border ${getCategoryStyles(prod.category)}`}>
                                {prod.category}
                            </span>
                            <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-[0.1em] border ${prod.status === 'ok' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-orange-500/10 text-orange-400 border-orange-500/20 shadow-[0_0_10px_rgba(251,146,60,0.1)]'}`}>
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-px bg-slate-800/40 border border-slate-800/40 rounded-2xl overflow-hidden">
                        <div className="bg-slate-950/60 p-4 flex flex-col gap-1">
                            <p className="text-[8px] text-slate-600 font-black uppercase tracking-widest italic flex items-center gap-1.5"><Warehouse size={10} /> Saldo Operacional</p>
                            <p className="text-[11px] font-black text-white italic">{formatNumber(prod.stock)} <span className="text-slate-500 uppercase">{prod.unit}</span></p>
                        </div>
                        <div className="bg-slate-950/60 p-4 flex flex-col gap-1">
                            <p className="text-[8px] text-slate-600 font-black uppercase tracking-widest italic flex items-center gap-1.5"><DollarSign size={10} /> Valorização Un.</p>
                            <p className="text-[11px] font-black text-emerald-400 italic">{formatCurrency(prod.price, settings.currency)}</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        <div className="h-1.5 w-full bg-slate-950 rounded-full overflow-hidden border border-slate-900">
                            <div
                                className={`h-full transition-all duration-1000 ${prod.status === 'ok' ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-orange-500 shadow-[0_0_8px_#f97316]'}`}
                                style={{ width: `${Math.min((Number(prod.stock) / (Math.max(Number(prod.minStock), 1) * 2)) * 100, 100)}%` }}
                            />
                        </div>
                        <div className="flex justify-between items-center px-1">
                            <p className="text-[8px] text-slate-700 font-black uppercase tracking-widest italic">Nível de Contingência</p>
                            <span className="text-[8px] font-black text-slate-500 italic uppercase">Min: {formatNumber(prod.minStock)} {prod.unit}</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-950/50 p-4 rounded-[1.25rem] border border-slate-900 space-y-3 group-hover:border-emerald-500/20 transition-all">
                        <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest">
                            <span className="text-slate-700 italic flex items-center gap-1.5"><MapPin size={10} /> {prod.location || 'GLOBAL_STK'}</span>
                            <span className="text-emerald-500/50 italic">REF: {prod.batch || 'AUTO_GEN'}</span>
                        </div>
                        <div className="flex justify-between items-center">
                            <div className="space-y-0.5">
                                <p className="text-[8px] text-slate-700 font-black uppercase tracking-widest italic leading-none">Patrimônio Lote</p>
                                <p className="text-[11px] font-black text-white italic tracking-tight">{formatCurrency(Number(prod.stock) * Number(prod.price), settings.currency)}</p>
                            </div>
