...
import { X, Plus, Trash2, Save, Info, Table, ChevronRight, AlertCircle, Layers, Keyboard, Settings2, RotateCcw, Calendar, Database } from 'lucide-react';
...
const SMART_MAP: Record<string, { category: string; unit: string; unitWeight: string }> = {
    'FERTILIZANTE': { category: 'Fertilizantes', unit: 'kg', unitWeight: '1,00' },
    'NPK': { category: 'Fertilizantes', unit: 'kg', unitWeight: '1,00' },
    'UREIA': { category: 'Fertilizantes', unit: 'kg', unitWeight: '1,00' },
    'SEMENTE': { category: 'Sementes', unit: 'sc (saco)', unitWeight: '50,00' },
    'MILHO': { category: 'Sementes', unit: 'sc (saco)', unitWeight: '50,00' },
    'SOJA': { category: 'Sementes', unit: 'sc (saco)', unitWeight: '50,00' },
    'GLIFOSATO': { category: 'Defensivos', unit: 'L', unitWeight: '1,00' },
    'HERBICIDA': { category: 'Defensivos', unit: 'L', unitWeight: '1,00' },
    'DIESEL': { category: 'Combustível', unit: 'L', unitWeight: '1,00' },
    'ÓLEO': { category: 'Combustível', unit: 'L', unitWeight: '1,00' },
    'PEÇA': { category: 'Peças', unit: 'un', unitWeight: '1,00' },
    'FILTRO': { category: 'Peças', unit: 'un', unitWeight: '1,00' },
};
...
export const BulkAuditForm: React.FC<BulkAuditFormProps> = ({
    ...
    const initialEntry = { tempId: Date.now(), name: '', category: 'Fertilizantes', unit: 'kg', unitWeight: '1,00', stock: '', price: '', minStock: '', batch: '', expirationDate: '', location: '' };
    ...
    const addRow = useCallback(() => {
        setEntries(prev => [
            ...prev,
            { tempId: Date.now(), name: '', category: globalCategory, unit: globalUnit, unitWeight: '1,00', stock: '', price: '', minStock: '', batch: '', expirationDate: '', location: '' }
        ]);
            ...
            <Card variant="glass" className="w-full max-w-[98vw] h-[92vh] relative z-10 p-0 overflow-hidden border-emerald-500/20 shadow-2xl rounded-[2.5rem] flex flex-col">
                ...
                <div className="p-8 border-b border-slate-800 bg-slate-950/40 flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] mt-1 pr-3 border-r border-slate-800 inline-block mr-3 leading-none">SpeedGrid™ v4.0 (Intelligence)</p>
                            <span className="text-[10px] text-emerald-500/60 font-black uppercase tracking-widest leading-none">Audit Engine Ativado</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-8">
                        <button
                            onClick={clearGrid}
                            className="px-4 py-2 rounded-xl bg-orange-500/10 text-orange-400 border border-orange-500/20 text-[9px] font-black uppercase tracking-widest transition-all hover:bg-orange-500/20 flex items-center gap-2"
                        >
                            <RotateCcw size={14} /> RESET DE DECK
                        </button>
                        <button
                            onClick={() => setShowAdvanced(!showAdvanced)}
                            className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all flex items-center gap-2 ${showAdvanced ? 'bg-blue-500 text-blue-950 border-blue-400' : 'bg-slate-900 text-slate-500 border-slate-800 hover:text-white'}`}
                        >
                        ...
                        {(hasDuplicates || entries.some(e => checkNameExists(e.name))) && (
                            <div className="flex items-center gap-2 px-4 py-2 bg-rose-500/10 border border-rose-500/20 rounded-xl animate-pulse">
                                ...
                                <span className="text-[10px] text-rose-400 font-black uppercase tracking-widest">Conflitos de Integridade</span>
                            </div>
                        )}
                ...
                <div className="p-6 bg-slate-900/30 border-b border-slate-800/60 flex flex-wrap gap-6 items-end shrink-0">
                    <div className="space-y-2">
                        <label className="text-[9px] text-slate-500 uppercase font-black tracking-widest ml-1">Preset Categoria</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-[9px] text-slate-500 uppercase font-black tracking-widest ml-1">Preset Unidade</label>
                    ...
                    <button onClick={applyGlobals} className="px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white text-[9px] font-black uppercase tracking-widest border border-slate-700 transition-all flex items-center gap-3 active:scale-95 shadow-lg group">
                    ...
                    <table className="w-full border-collapse">
                        <thead className="sticky top-0 z-20 bg-slate-950/90 backdrop-blur-md">
                            <tr className="border-b border-slate-800 text-left">
                                <th className="p-5 text-[9px] font-black text-slate-500 uppercase tracking-widest w-12 text-center sticky left-0 bg-slate-950 z-30">#</th>
                                <th className="p-5 text-[9px] font-black text-slate-500 uppercase tracking-widest min-w-[300px] sticky left-12 bg-slate-950 z-30">Nomenclatura Técnica</th>
                                <th className="p-5 text-[9px] font-black text-slate-500 uppercase tracking-widest w-48">Categoria</th>
                                <th className="p-5 text-[9px] font-black text-slate-500 uppercase tracking-widest w-40">Unidade</th>
                                <th className="p-5 text-right text-[9px] font-black text-emerald-400 uppercase tracking-widest w-36 text-right">Conversor Ref. (KG/L)</th>
                                <th className="p-5 text-right text-[9px] font-black text-slate-500 uppercase tracking-widest w-32">Saldo</th>
                                <th className="p-5 text-right text-[9px] font-black text-slate-500 uppercase tracking-widest w-36">Custo ({settings.currency})</th>
                                <th className="p-5 text-right text-[9px] font-black text-orange-400 uppercase tracking-widest w-32">Mínimo</th>
                                {showAdvanced && (
                                    <>
                                        <th className="p-5 text-[9px] font-black text-slate-500 uppercase tracking-widest w-40">Protocolo Lote</th>
                                        <th className="p-5 text-[9px] font-black text-slate-500 uppercase tracking-widest w-40">Vencimento</th>
                                        <th className="p-5 text-[9px] font-black text-slate-500 uppercase tracking-widest w-48">Galpão / Local</th>
                                    </>
                                )}
                                <th className="p-5 text-center text-[9px] font-black text-slate-500 uppercase tracking-widest w-16 sticky right-0 bg-slate-950 z-30"></th>
                            </tr>
                        </thead>
                            ...
                            {entries.map((entry, idx) => {
                                ...
                                return (
                                    <tr key={entry.tempId} className={`hover:bg-slate-800/40 transition-colors group ${namesInTerm > 1 || existsInDB ? 'bg-rose-500/5' : ''}`}>
                                        <td className="p-5 text-center text-[10px] font-black text-slate-700 italic group-hover:text-emerald-500 transition-colors sticky left-0 bg-slate-950/40 backdrop-blur-md z-10">{idx + 1}</td>
                                        <td className="p-2 sticky left-12 bg-slate-950/40 backdrop-blur-md z-10">
                                            <div className="relative group/name">
                                                <input
                                                    ...
                                                    className={`w-full bg-transparent border-none px-4 py-3 text-[11px] font-black text-white outline-none focus:bg-slate-900/80 rounded-xl placeholder:text-slate-800 italic transition-all ${namesInTerm > 1 ? 'text-rose-400' : existsInDB ? 'text-amber-400' : ''}`}
                                                />
                                                {existsInDB && (
                                        ...
                                        <td className="p-2">
                                            <select
                                                ...
                                                className="w-full bg-transparent border-none px-4 py-3 text-[10px] font-black text-slate-400 outline-none focus:bg-slate-900/80 rounded-xl cursor-pointer hover:text-white transition-all appearance-none"
                                            >
                                                <optgroup label="AGRÍCOLAS" className="bg-slate-900">
                                                    <option>Fertilizantes</option><option>Sementes</option><option>Defensivos</option><option>Herbicidas</option>
                                                    <option>Fungicidas</option><option>Inseticidas</option><option>Adjuvantes</option><option>Corretivos</option><option>Nutrição Foliar</option>
                                        ...
                                        <td className="p-2 text-right">
                                            <input
                                                ...
                                                placeholder="1,00"
                                                className="w-full bg-transparent border-none px-4 py-3 text-[11px] font-black text-emerald-400 text-right outline-none focus:bg-slate-900/80 rounded-xl transition-all"
                                            />
                                        </td>
                                        <td className="p-2 text-right">
                                            <input
                                                ...
                                                placeholder="0,00"
                                                className={`w-full bg-transparent border-none px-4 py-3 text-[11px] font-black text-right outline-none focus:bg-slate-900/80 rounded-xl transition-all ${parseValue(entry.stock) > 0 && parseValue(entry.stock) <= parseValue(entry.minStock) ? 'text-orange-400' : 'text-white'}`}
                                            />
                                        </td>
                                        <td className="p-2 text-right">
                                            <input
                                                ...
                                                placeholder="0,00"
                                                className="w-full bg-transparent border-none px-4 py-3 text-[11px] font-black text-emerald-400 text-right outline-none focus:bg-slate-900/80 rounded-xl transition-all italic"
                                            />
                                        </td>
                                        <td className="p-2 text-right">
                                            <input
                                                ...
                                                placeholder="0,00"
                                                className="w-full bg-transparent border-none px-4 py-3 text-[11px] font-black text-orange-500 text-right outline-none focus:bg-slate-900/80 rounded-xl transition-all"
                                            />
                                        </td>
                                        {showAdvanced && (
                                            <>
                                                <td className="p-2">
                                                    <input
                                                        ...
                                                        className="w-full bg-transparent border-none px-4 py-3 text-[11px] font-black text-white outline-none focus:bg-slate-900/80 rounded-xl"
                                                    />
                                                </td>
                                                <td className="p-2">
                                                    <div className="relative">
                                                        <input
                                                            ...
                                                            className={`w-full bg-transparent border-none px-4 py-3 text-[11px] font-black outline-none focus:bg-slate-900/80 rounded-xl uppercase transition-all ${isExpiredDate ? 'text-rose-500 bg-rose-500/5' : 'text-blue-400'}`}
                                                        />
                                                        {isExpiredDate && (
                                                ...
                                                <td className="p-2">
                                                    <input
                                                        ...
                                                        className="w-full bg-transparent border-none px-4 py-3 text-[11px] font-black text-slate-400 outline-none focus:bg-slate-900/80 rounded-xl italic"
                                                    />
                                                </td>
                                            </>
                                        )}
                                        <td className="p-2 text-center sticky right-0 bg-slate-950/40 backdrop-blur-md z-10">
                                            <button onClick={() => removeRow(entry.tempId)} className="w-10 h-10 flex items-center justify-center text-slate-700 hover:text-rose-500 transition-all opacity-0 group-hover:opacity-100 bg-slate-900/40 rounded-lg hover:bg-rose-500/10">
                                                <Trash2 size={16} />
                                            </button>
                                        </td>
                                    </tr>
